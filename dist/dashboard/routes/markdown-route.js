...
var reader = new MdxReader({
  contentDir: import_path2.default.join(process.cwd(), process.env.TA_CONTENT_DIR ?? "content"),
  stripSegments: (process.env.TA_STRIP_SEGMENTS ?? "").split(",").map((s) => s.trim()).filter(Boolean)
});
var renderer = new MarkdownRenderer();
var cache = new CacheManager({
  cacheDir: import_path2.default.join(process.cwd(), process.env.TA_DATA_DIR ?? "data", "ta-cache")
});
