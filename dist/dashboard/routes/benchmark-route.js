...
// src/dashboard/routes/benchmark-route.ts
var benchmark_route_exports = {};
__export(benchmark_route_exports, {
  GET: () => GET,
  POST: () => POST
});
module.exports = __toCommonJS(benchmark_route_exports);
...
async function GET(req) {
  ...
  const filters = {
    competitor_url: p.get("competitor_url") ?? void 0,
    ai_platform: p.get("ai_platform") ?? void 0,
    sinceDate: p.get("sinceDate") ?? void 0
  };
  ...
  return import_server.NextResponse.json({ competitors, rows: paginated, total, analytics });
}
async function POST(req) {
  ...
  if (action === "save_competitors") {
    ...
    if (!Array.isArray(list)) return import_server.NextResponse.json({ error: "Invalid competitors list" }, { status: 400 });
    ...
    return import_server.NextResponse.json({ ok: true, competitors: deduped });
  }
  if (action === "record") {
    const r = body.record;
    if (!r?.competitor_url || !r?.test_prompt || !r?.ai_platform || !r?.test_date) {
      return import_server.NextResponse.json({ error: "Missing required fields" }, { status: 400 });
    }
  ...
  if (action === "delete") {
    ...
    if (!id) return import_server.NextResponse.json({ error: "Missing id" }, { status: 400 });
...
function buildAnalytics(rows) {
  ...
  for (const r of rows) {
    ...
    if (r.cited_rank !== null) {
      byCompetitor[key].cited++;
      byCompetitor[key].totalRank += r.cited_rank;
      byCompetitor[key].rankCount++;
    }
...
//# sourceMappingURL=benchmark-route.js.map